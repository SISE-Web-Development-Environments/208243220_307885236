...
listOfPlayers['p'] = { password: 'Ppp123', fullName: "pp pp", email:"pp@gmail.com", birthDate: "12-04-2020"};
...
{
    if(required(userNameLogIn) && checkpassword(passwordLogIn)){
...
function validation(userNameReg,passwordReg,fullNameReg,emailReg,birthDateReg){
    ...
    if(required(userNameReg) && checkpassword(passwordReg) && checkFullName(fullNameReg)&& checkEmail(emailReg)&& checkDate(birthDateReg)){
      //insert into list of players
        listOfPlayers[userNameReg.value]={password: passwordReg.value, fullName: fullNameReg.value, email: emailReg.value,birthDate: birthDateReg.value};
